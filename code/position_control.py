...
COUNTS_PER_REV = 30 * GEAR_RATIO  # The counts per revolution of the motor's output shaft
...
POSITION_EXTENT = 360                   # How far from zero to move the motor, in degrees
INTERP_MODE = 2                         # The interpolating mode between setpoints. STEP (0), LINEAR (1), COSINE (2)

# PID values
POS_KP = 0.3                         # Position proportional (P) gain
POS_KI = 0.01                            # Position integral (I) gain
POS_KD = 0.02                        # Position derivative (D) gain
...
while not user_sw.raw():
    ...
    if print_count == 0:
        print("Dist =", measure_distance(), end=", ")
    ...
    if update >= UPDATES_PER_MOVE:
        ...
        if start_value==POSITION_EXTENT:
            end_value = 0
        else:
            end_value = POSITION_EXTENT
